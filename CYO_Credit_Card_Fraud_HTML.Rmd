...
```{r keras install, message=FALSE, warning=FALSE}
...
```{r package install, message=FALSE, warning=FALSE}
...
```{r getting dataset, message=FALSE, warning=FALSE}
#Creating data set
#Creating data set
...
```{r Rate of Fraud, message=FALSE, warning=FALSE}
table(data$Class) %>% 
...
```{r proportion of fraud, message=FALSE, warning=FALSE}
...
```{r Amount v Class, message=FALSE, warning=FALSE}
#amount v Class
data %>% 
...
```{r test val, message=FALSE, warning=FALSE}
...
```{r data rose, message=FALSE, warning=FALSE}
...
```{r data rose fraud, message=FALSE, warning=FALSE}
...
```{r changing levels, message=FALSE, warning=FALSE}
...
```{r TrainControl, message=FALSE, warning=FALSE}
## Set traincontrol  
...
```{r GLM System, message=FALSE, warning=FALSE}
...
```{r Ranger System, message=FALSE, warning=FALSE}
...
```{r KNN3 System, message=FALSE, warning=FALSE}
...
```{r baking, message=FALSE, warning=FALSE}
...
```{r Fitting Keras, message=FALSE, warning=FALSE}
history <- fit(
  object = seq_mod,
  x = as.matrix(x_train),
  y = y_train,
...
```{r loading weights, message=FALSE, warning=FALSE}
...
```{r keras results, message=FALSE, warning=FALSE}
keras_cf
...
```{r Preprocess Validate, message=FALSE, warning=FALSE}
...
```{r Loading Weights Val, message=FALSE, warning=FALSE}
latest <- tf$train$latest_checkpoint(checkpoint_dir)
...
```{r Final Validation Predict, message=FALSE, warning=FALSE}
...
```{r Val CF, message=FALSE, warning=FALSE}
val_cf 
...
On the test set, the keras Sequential model performed the best, with a close to perfect 0.999527, however, when tested on the validation set, this score dropped to below 0.92. However, it must be noted that within all the systems evaluated, there is scope for improvements and further refinements.
...
```{r Conclusions Table, message=FALSE, warning=FALSE}
